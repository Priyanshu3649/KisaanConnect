 ...
 *
 * - cropAiDiagnosis - A function that handles the plant disease detection process.
 * - CropAiDiagnosisInput - The input type for the cropAiDiagnosis function.
 * - CropAiDiagnosisOutput - The return type for the cropAiDiagnosis function.
 */

import { ai } from '@/ai/genkit';
import { z } from 'genkit';

const CropAiDiagnosisInputSchema = z.object({
  photoDataUri: z
    .string()
    .describe(
...
export type CropAiDiagnosisInput = z.infer<typeof CropAiDiagnosisInputSchema>;

const CropAiDiagnosisOutputSchema = z.object({
...
export type CropAiDiagnosisOutput = z.infer<typeof CropAiDiagnosisOutputSchema>;


const prompt = ai.definePrompt({
  name: 'cropAiDiagnosisPrompt',
  input: { schema: CropAiDiagnosisInputSchema },
  output: { schema: CropAiDiagnosisOutputSchema },
...
export const cropAiDiagnosis = ai.defineFlow(
  {
    name: 'cropAiDiagnosisFlow',
    inputSchema: CropAiDiagnosisInputSchema,
    outputSchema: CropAiDiagnosisOutputSchema,
  },
  async (input) => {
    try {
      ...
      return output;
        ...
        console.error("Error in cropAiDiagnosisFlow:", error);
